...
import my_controller as myCtrl
...
class LLDPSender (object):
  ...
  def _handle_openflow_ConnectionUp (self, event):
    ...
    self._next_cycle.append(LLDPSender.SendItem(event.dpid,None,
           self.create_packet_out(event.dpid,of.OFPP_FLOOD,None)))
    self._set_timer()
    # Already written in Discoveri._handle_openflow_ConnectionUp
    # sDPID= dpid_to_str(event.dpid)
    # myCtrl.my_add_switch(sDPID)
    # # log.debug("added switch with DPID: %i",sDPID)
    # for p in event.ofp.ports:
    #     myCtrl.switch[sDPID].dpid_port[p.port_no]=event.dpid
    #     myCtrl.switch[sDPID].port_mac[p.port_no]=p.hw_addr
    #     #useless, but I insert as Matteo provided it. TODO to be discussed
    # myCtrl.switch[sDPID].mac_port[p.hw_addr]=p.port_no

            #old way of discovery
  ...
  def add_switch (self, dpid):
      self._next_cycle.append(LLDPSender.SendItem(dpid,None,
            self.create_packet_out(dpid,OFPT.OFPP_FLOOD,None)))
      if set_timer:self._set_timer()
  ...
  def create_packet_out (self, dpid, port_num, port_addr):
    ...
    #new way. the receiver will know the packet
    po = of.ofp_packet_out(action = of.ofp_action_output(port=of.OFPP_FLOOD))
    po.data = eth.pack()
    return po.pack()
  ...
  def _create_discovery_packet (dpid, port_num, port_addr, ttl):
    ...
    eth.src = pkt.ETHERNET.ETHER_BROADCAST #changed by the switch @see Discovery.installFlow
...
class Discovery (EventMixin):
  ...
  def install_flow (self, con_or_dpid, priority = None, dpid=None):
    if priority is None:
    ...
    con.send(msg)
    if dpid is not None:
        #ADD modification to change src mac address from ff:ff:ff:ff:ff:ff to the one
        #associated with the corrisponding port
        changeMacMatch = of.ofp_match(dl_type = pkt.ethernet.LLDP_TYPE,
                in_port = of.OFPP_CONTROLLER)
        secondMsg=of.ofp_flow_mod()
        secondMsg.priority=priority+100
        secondMsg.match = changeMacMatch
        for prt,mac in  myCtrl.switch[dpid_to_str(dpid)].port_mac.items():
            log.debug("switch %s has port #%i with mac %s",dpid_to_str(dpid),prt,mac.to_str())
            setSource=of.ofp_action_dl_addr.set_src(mac)
            secondMsg.actions.append(setSource) #rewrite mac
            secondMsg.actions.append(of.ofp_action_output(port = prt)) #send pkt to port
        con.send(secondMsg)
    return True
  ...
  def _handle_openflow_ConnectionUp (self, event):
    if self._install_flow:
      ...
      sDPID= dpid_to_str(event.dpid)
      myCtrl.my_add_switch(sDPID)
      log.debug("added switch with DPID: %s",sDPID)
      for p in event.ofp.ports:
          if(p.port_no<6000):
            myCtrl.switch[sDPID].dpid_port[p.port_no]=event.dpid
            myCtrl.switch[sDPID].port_mac[p.port_no]=p.hw_addr
            ...
            pass
      self.install_flow(event.connection,priority=1, dpid=event.dpid)
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    if originatorDPID not in core.openflow.connections:
      ...
      return EventHalt
      log.debug("originatorDPID: %i",originatorDPID)
    # old # Get port number from port TLV
    ...
    #   except:
    #     pass
    originatorPort=None #initialization
    sDPID = dpid_to_str(originatorDPID)
    origSwitch =myCtrl.switch[sDPID]
    if packet.src== pkt.EthAddr("ff:ff:ff:ff:ff:ff"):
        log.warning("Mac address of switch %s not changed",dpid_to_str(originatorDPID))
    else:
        originatorPort = origSwitch.mac_port[packet.src]
        log.debug("originatorPort: %i",originatorPort)
