...
\begin{itemize}
 ...
 \item \texttt{priority:} priority in case of rows with more than one correspondence;
 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \item \texttt{counter:} this field was improved every time a correspondence with a packet was matched;
 \item \texttt{istructions:} a set of action that the switch add on \emph{Action-set} of packet or modification on pipeline flux;
 \item \texttt{timeouts:} this is the maximum time afther that the rule exired;
 ...
 \item \texttt{flags:} this field is use to differentiate the management of the voice;
 \\
 \\
\end{itemize}
...
these are theprincipal match method that we can use with OpenFlow v1.0:
\begin{itemize}
...
With the OpenFlow v1.0 in practice we can do simply things for example:
\begin{itemize}
...
From OpenFlow v1.1 on we can modify Time To Live and other things that allow us to do something more.
...
some types of flows, so we used our PCs to sniff the traffic. Traffic analysis will be discussed in section.
  \#TODO stato dell arte e paper di riferimento  prova a darci un occhio adrea please.
...
in orther to understand in a practical level how to implement a \textit{Quality of Service} in a SDN scenarios.
...
A general efficient Network has to manage some tasks for example: avoid links congestion, reroute traffic when some malfunction accure on a link
and running some link path algorithm. 
In addition our network has to be adaptable in orther to ensure a QoS type of service.
...
automatically updatable, where the link statistics can be saved to allow the best path for a data stream. 
...
reserved for the connection with the controller, consequently we have to connect both host and other routerboards using only 4 ports.
...
\begin{itemize}
 \item \texttt{pox.openflow.discovery.launch()} this function runs an algorithm in order to discovery the network. Every time a new
 topological change occur this function returns an event on POX with which we can update our topological knowledge.
...
\subsection{Graph and Network structures}
  In order to achieve the realtime adaptability and statistics storage we have developed ad-hoc structure.
  ...
  This topological structure is updated live thanks to connection event and link discover offered by POX's libraries:
  \begin{itemize}
   \item \texttt{_handle_LinkEvent}: from openflow.discovery an event of this type give us two type of information; a new link was added and a still present link was expired;
   \item \texttt{_handle_ConnectionUp}: from core.openflow this event join up when a new switch was added on the network and give us
   some information of the switch, in particular its ports configuration;
   \item \texttt{_handle_ConnectionUp}: from core.openflow like the event explained above comes up when a switch was disconnected from the network;
   \item \texttt{_handle_port_status}: from core.openflow this event occure when on a particular switch there is a change on its port, for example
   some host connect or the link was expired;\\
  \end{itemize}

  The link on our network besides have many parameters in witch we can save network performance. 
  In orther to update every link parameters we had to creat ad hoc function
